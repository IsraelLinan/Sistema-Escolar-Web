...
export default function PersonalAdminIngress() {
  ...
  const handleAction = async (tipo) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/auxiliares/${tipo}`, { codigo_barras: codigo });
      setStatus({
  ...
  const statusColors = {
    success: 'bg-[#f43f5e20] border-[#f43f5e] text-[#f43f5e]',
    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-theme2 border border-theme rounded-2xl p-6 mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-theme text-xl font-bold">Asistencia de Personal Administrativo</h2>
      ...
      <div className="bg-theme2 border border-theme rounded-2xl p-6">
        ...
        <div className="text-center mb-4">
          <span className="text-5xl text-[#f43f5e]">▤</span>
        ...
        <input
          ...
          className="w-full bg-theme3 border border-[#f43f5e] text-theme rounded-xl px-4 py-3 text-sm font-mono focus:outline-none mb-6"
        />

        <div className="flex gap-3 mb-6">
          <button
            onClick={() => handleAction('ingreso')}
            className="flex-1 bg-[#f43f5e] hover:bg-[#e11d48] text-white font-bold py-3 rounded-xl transition text-sm"
          >
            ▶ Registrar Ingreso
          </button>
          <button
