...
export default function FotocheckAuxiliar() {
  ...
  const buscarCodigo = async () => {
    if (!nombre.trim()) {
      setError('Ingrese el nombre del auxiliar primero.');
      return;
    }
    ...
    try {
      ...
      setError('No se encontró código para este auxiliar. Genéralo primero en el módulo de códigos.');
    } finally {
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      pdf.save(`fotocheck_auxiliar_${nombre || 'auxiliar'}.pdf`);
    } catch (e) {
  ...
  const guardarFotocheck = async () => {
    if (!nombre.trim()) {
      setError('Ingrese el nombre del auxiliar.');
      return;
    }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-theme2 border border-theme rounded-2xl p-6 mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-theme text-xl font-bold">Generar Fotocheck Auxiliar</h2>
            <p className="text-muted text-sm">Diseña e imprime el carné del auxiliar</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-theme2 border border-theme rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <label className="w-full flex items-center gap-3 bg-theme3 border border-dashed border-theme hover:border-[#ef4444] text-muted rounded-xl px-4 py-3 text-sm cursor-pointer transition">
              <span>🏫</span>
          ...
          {/* Nombre auxiliar */}
          <div>
            <label className="block text-muted text-xs font-bold uppercase mb-1">Nombre del Auxiliar</label>
            <input
              type="text"
              value={nombre}
              onChange={e => setNombre(e.target.value)}
              placeholder="Ej: García López, Juan Carlos"
              className="w-full bg-theme3 border border-theme text-theme rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#ef4444]"
            />
          </div>
          ...
          <div>
            ...
            >
              {loadingCodigo ? 'Buscando...' : '🔍 Buscar código del auxiliar'}
            </button>
            {imagenCodigo && (
          ...
          <div className="flex gap-3">
            <button
              onClick={exportarPDF}
              disabled={loadingPDF}
              className="flex-1 bg-[#ef4444] hover:bg-[#dc2626] text-white font-bold py-3 rounded-xl transition text-sm disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex-1 bg-[#dc2626] hover:bg-[#b91c1c] text-white font-bold py-3 rounded-xl transition text-sm disabled:opacity-50"
            >
          ...
          >
            ...
            <div style={{ background: 'white', height: '50%', position: 'relative' }}>
              ...
              <div style={{
                ...
                <div><span style={{ color: '#666' }}>Cargo   :</span> <strong>Auxiliar</strong></div>
            ...
            <div style={{
              ...
              {foto ? (
                ...
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '32px', background: '#fee2e2'
                }}>👤</div>
              )}
            </div>
